...
const DevBingo = ({ setActive }) => {
  ...
  return (
    <div className="space-y-6 p-4 sm:p-6 relative">
      ...
      {/* ─── Bingo Grid Board ────────────────────────────────────── */}
      <div className="grid grid-cols-5 gap-1.5 sm:gap-3 max-w-3xl mx-auto py-2">
        {board.map((tile, idx) => {
          ...
          return (
            <button
              ...
              className={`aspect-square rounded-xl p-1 sm:p-2.5 flex flex-col items-center justify-center text-center transition-all duration-350 border relative ${
                tile.isFree
                  ? 'border-amber-500/35 bg-amber-500/10 text-amber-300 font-bold animate-pulse'
                  : isChecked
                    ? 'border-cyan-400 bg-cyan-500/20 text-cyan-105 shadow-lg shadow-cyan-400/5'
            ...
            >
              ...
              <span className={`absolute top-0.5 left-1 min-[370px]:top-1 min-[370px]:left-1.5 text-[6.5px] min-[370px]:text-[8px] font-mono select-none ${isChecked ? 'text-cyan-400 font-bold' : 'text-slate-600'}`}>
                {String.fromCharCode(65 + Math.floor(idx / 5))}{(idx % 5) + 1}
              </span>
              
              {/* Habit description text */}
              <span className={`text-[6.5px] min-[370px]:text-[8px] sm:text-[10px] md:text-xs leading-tight font-mono break-words w-full px-0.5 flex-1 flex items-center justify-center ${tile.isFree ? 'text-center font-bold text-amber-300' : 'text-slate-200'}`}>
                {tile.text}
              </span>

              {/* Check indicator dot */}
              <div className={`absolute top-0.5 right-1 min-[370px]:top-1 min-[370px]:right-1.5 h-1.5 w-1.5 sm:h-2 sm:w-2 rounded-full border transition-all ${
                tile.isFree 
